...
<style>
    ...
    .select2-container--default .select2-selection--single .select2-selection__rendered {
        line-height: 28px !important; /* Adjust as needed */
    }
    .select2-dropdown {
...
<form id="posForm" action="{{ route('pengurus.transaksi-pembelian.store') }}" method="POST" class="animate-fade-in" data-validate>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white/80 backdrop-blur-lg p-6 rounded-2xl shadow-lg border border-white/20">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="user_id" id="user_id" class="w-full select2-basic" required>
                            <option value="">Pilih Anggota</option>
                            @foreach($anggota as $agt)
                                <option value="{{ $agt->id }}" data-nomor="{{ $agt->nomor_anggota ?? '-' }}">{{ $agt->name }} ({{ $agt->nomor_anggota ?? 'N/A' }})</option>
                            @endforeach
                        </select>
                        @error('user_id') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label for="tanggal_pembelian" class="block text-sm font-medium text-gray-700 mb-1.5">Tanggal Transaksi <span class="text-red-500">*</span></label>
                        <input type="date" name="tanggal_pembelian" id="tanggal_pembelian" value="{{ old('tanggal_pembelian', date('Y-m-d')) }}" 
                               class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500" required>
            ...
            <div class="bg-white/80 backdrop-blur-lg p-6 rounded-2xl shadow-lg border border-white/20">
                ...
                <div>
                    ...
                    <select id="barang_search" class="w-full select2-basic">
                ...
                <div class="mt-6 overflow-x-auto">
                    <h4 class="text-md font-semibold text-gray-700 mb-3">Keranjang Belanja</h4>
                    <table class="w-full min-w-[600px]">
                        ...
                        <tbody id="cartItemsBody">
                            ...
                            <tr id="cartEmptyRow">
                                <td colspan="5" class="text-center py-6 text-gray-400 italic">Keranjang masih kosong.</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
        ...
        <div class="lg:col-span-1">
            <div class="bg-white/80 backdrop-blur-lg p-6 rounded-2xl shadow-lg border border-white/20 sticky top-24"> {{-- sticky top untuk ringkasan --}}
                ...
                <div class="space-y-3 mb-6 text-sm">
                    <div class="flex justify-between">
                        ...
                        <span class="font-semibold text-gray-800" id="cartSubtotal">Rp 0</span>
                    </div>
                    {{-- Bisa ditambahkan diskon, pajak jika ada --}}
                    <div class="flex justify-between text-lg border-t pt-3">
                        <span class="font-bold text-gray-800">Total Belanja:</span>
                        <span class="font-bold text-blue-600" id="cartTotal">Rp 0</span>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <select name="metode_pembayaran" id="metode_pembayaran" class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500" required>
                        ...
                        <option value="hutang">Hutang / Cicilan</option>
                    </select>
                    @error('metode_pembayaran') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>

                <div id="pembayaranTunaiFields" class="mt-4 space-y-4">
                    <x-forms.input type="number" name="total_bayar_manual" label="Jumlah Bayar (Tunai)" placeholder="0" attributes="min=0 step=any" />
                ...
                <div id="infoSaldoSukarela" class="mt-4 hidden bg-blue-50 p-3 rounded-lg text-sm text-blue-700">
                    Saldo sukarela anggota akan dicek saat proses simpan. Pastikan saldo mencukupi.
                    <p>Saldo Terkini: <span id="displaySaldoSukarelaAnggota" class="font-semibold">-</span></p>
                </div>
                
                <div id="infoHutangFields" class="mt-4 hidden">
                     <x-forms.input type="number" name="uang_muka" label="Uang Muka (DP)" placeholder="0" attributes="min=0 step=any" />
                     <p class="text-xs text-gray-500 mt-1">Kosongkan jika tidak ada uang muka. Transaksi akan menjadi hutang penuh.</p>
                </div>


                <div class="mt-6">
                    <label for="catatan" class="block text-sm font-medium text-gray-700 mb-1.5">Catatan (Opsional)</label>
                    <textarea name="catatan" id="catatan" rows="3" class="w-full px-4 py-2 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500" placeholder="Catatan tambahan untuk transaksi ini...">{{ old('catatan') }}</textarea>
                </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function renderCart() {
        ...
        itemsJsonInput.value = JSON.stringify(cart.map(item => ({ barang_id: item.id, jumlah: item.jumlah, harga_satuan: item.harga })));
    ...
    function updateItemQuantity(index, newQuantity) {
        ...
        if (newQuantity > 0 && newQuantity <= item.stok_awal) {
            ...
            KoperasiApp.showNotification(`Jumlah melebihi stok tersedia (${item.stok_awal}) untuk ${item.nama}.`, 'warning');
        }
        renderCart();
    }
    ...
    barangSearchEl.on('select2:select', function (e) {
        ...
        if (existingItemIndex > -1) {
            ...
            if (item.jumlah < item.stok_awal) {
                ...
                KoperasiApp.showNotification(`Stok ${item.nama} sudah maksimal di keranjang.`, 'warning');
            }
        } else {
    ...
    function updatePembayaranFields() {
        ...
        const totalBelanja = parseFloat(cartTotalEl.textContent.replace(/[^0-9.-]+/g,"").replace('.', '')) || 0;
    ...
    async function fetchSaldoSukarelaAnggota() {
        ...
        //     });
        displaySaldoSukarelaAnggotaEl.textContent = 'Rp XXX (Fitur ambil saldo belum aktif)'; // Placeholder
    }
